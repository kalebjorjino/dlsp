<?php
    class Usuario extends Conectar{
        ...
        public function get_usuario_total_x_id($us_id){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT COUNT(*) as TOTAL FROM tm_registro where us_id = ?";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $us_id);
            $sql->execute();
            return $resultado=$sql->fetchAll();
        ...
        public function get_usuario_grafico($us_id){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT tm_cargo.carg_nom as nom,COUNT(*) AS total
                FROM   tm_registro  JOIN  
                    tm_cargo ON tm_registro.carg_id = tm_cargo.carg_id  
                WHERE    
                tm_registro.est = 1
